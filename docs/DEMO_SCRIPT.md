...
- [ ] `--live` only if you're going for it: `.env` has `ANTHROPIC_API_KEY`; one warm-up run done. **If anything about live is shaky, skip it — the fixtures are the demo.**
...
**2:20 — (Optional flex) live upload.** *(only if green)*
> "Want to break it? Hand me an antique." *(upload → Claude sees it → fresh field-guide page,
> live)* "Real photo, real classification, same frozen math."
